...
CREATE TABLE users (
  ...
  address_state VARCHAR(50) NOT NULL, -- can change to just use state abbreivations so can be CHAR(2)?
...
CREATE TABLE availability (
  user_id VARCHAR(128) REFERENCES users(user_id) ON DELETE CASCADE, -- removed primary key from user_id here
...
CREATE TABLE language (
  ...
  PRIMARY KEY (user_id, language) -- lowkey forget we can do this... what stops us from replacing all 'UNIQUE' keywords with 'PRIMARY KEY'?
)
